...
### Export final results with precision, recall, f1-scores and run times in Results_R.csv 
### and export parameters in Params_r.csv.
...
output_file1 <- file.path(Starting_point, "test_results", "auto_osESD_test", "auto_osESD_Results_R.csv")
write.csv(result_cols, file = output_file1)
output_file2 <- file.path(Starting_point, "test_results", "auto_osESD_test", "auto_osESD_Params_R.csv")
write.csv(param_cols, file = output_file2)
setwd(dirname(rstudioapi::getSourceEditorContext()$path))
