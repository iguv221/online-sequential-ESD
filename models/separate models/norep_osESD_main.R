...
source("norep_osESD_Detector.R")
source("norep_osESD_Transform.R")
source("norep_osESD_Test.R")
...
# 
# t1 <- Sys.time()
# anoms <- norep_osESD_Detector(data=data$value, time=data$timestamps, opt$size, 
...
for (DATA_NUM in 1:100){
  ...
  t1 <- Sys.time()
  anoms <- norep_osESD_Detector(data=data$value, time=data$timestamps, train_size=opt$size,
                       dwin=opt$dwin, rwin=opt$rwin, alpha=opt$alpha, maxr=opt$maxr)
...
RESULTS
write.csv(RESULTS,file=("norep_osESD_Results_R.csv"))
