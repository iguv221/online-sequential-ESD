...
source("sosESD_Detector.R")
source("sosESD_Transform.R")
source("sosESD_Test.R")
...
# file_path <- paste('C:\\Users\\iguv2\\Desktop\\osESD_AllCodes\\data\\A3Benchmark-TS', opt$data, '.csv', sep='')
...
t1 <- Sys.time()
anoms <- sosESD_Detector(data=data$value,
                     wins=opt$size, alpha=opt$alpha, maxr=opt$maxr)
...
# RESULTS
# write.csv(RESULTS,file=("sosESD_Results_R.csv"))
# 
# 
