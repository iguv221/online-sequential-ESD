...
### 'Ablation_tests' is where all values will be saved and exported to csv when all datasets and models have been run.
### Each dataset uses 'data_results' to save its scores and this 'data_results' is appended to 'Ablation_tests'.
Ablation_tests<-c()
IDX<-1
for (dataset in Testing_Datasets){
    ...
    for (data_name in dir()){
        ...
        if (Model_Switch[1]==1){
            ...
            if (TRUE){
                png(file=paste0(Starting_point,"//test_results//ablation_test//plots//",Models[1],"_",gsub("//.csv", "", data_name),".png"))
        ...
        if (Model_Switch[2]==1){
            ...
            if (TRUE){
                png(file=paste0(Starting_point,"//test_results//ablation_test//plots//",Models[2],"_",gsub("//.csv", "", data_name),".png"))
        ...
        if (Model_Switch[3]==1){
            ...
            if (TRUE){
                png(file=paste0(Starting_point,"//test_results//ablation_test//plots//",Models[3],"_",gsub("//.csv", "", data_name),".png"))
        ...
        if (Model_Switch[4]==1){
            ...
            if (TRUE){
                png(file=paste0(Starting_point,"//test_results//ablation_test//plots//",Models[4],"_",gsub("//.csv", "", data_name),".png"))
        ...
        Ablation_tests<-rbind(Ablation_tests,data_results)
        IDX<-IDX+1
    }
    setwd('..')
}
...
colnames(Ablation_tests)<-Cols
Ablation_tests <- data.frame(Ablation_tests)
output_file <- file.path(Starting_point, "test_results", "ablation_test", "Ablation_Tests_results_R.csv")
write.csv(Ablation_tests, file = output_file)

original <- Ablation_tests

asdwadasdsd



datasets <- c('A1', 'A2', 'A3', 'A4', 'ARIMA', 'seasonal')
group_value <- function(value) {
  for (data_group_name in datasets) {
    if (grepl(data_group_name, value)) {
...
Ablation_tests$Group <- sapply(Ablation_tests$Data.Name, group_value)
grouped <- split(Ablation_tests, Ablation_tests$Group)
Averaged_Values <- list()
grouped

# names(group_data)[3:length(names(group_data))-1]
for (group_name in names(grouped)) {
    group_data <- grouped[[group_name]]
    group_values <- c(group_name)
    print(group_name)
    
    for (col in names(group_data)[3:length(names(group_data))-1]) {
        group_values <- c(group_values, round(mean(as.double(group_data[[col]]), na.rm = TRUE), 3))
    }
    print(group_values)
    # asdas
    Averaged_Values[[group_name]] <- group_values
}
Averaged_Values <- do.call(rbind, Averaged_Values)
Averaged_Values <- data.frame(Averaged_Values)
colnames(Averaged_Values) <- colnames(Ablation_tests)[2:(length(colnames(Ablation_tests)) - 1)]
output_file2 <- file.path(Starting_point, "test_results", "ablation_test", "Ablation_Tests_results_R_averaged_scores.csv")
write.csv(Averaged_Values, file = output_file2)
